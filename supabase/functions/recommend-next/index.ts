...
serve(async (req) => {
  ...
  try {
    ...
    const [patternsRes, tagsRes, campaignsRes, personasRes, profileRes] = await Promise.all([
      ...
      supabase.from("audience_personas").select("id, name, pain_points, goals, industry, geography").eq("user_id", user.id),
    ...
    recentTags.map((t: any) => t.persona_id).filter(Boolean).forEach((p: string) => { personaCounts[p] = (personaCounts[p] || 0) + 1; });
    ...
    const bestPatterns = patterns.filter((p: any) => p.sample_count >= 2)
      .sort((a: any, b: any) => b.avg_engagement_rate - a.avg_engagement_rate).slice(0, 5);
    const worstPatterns = patterns.filter((p: any) => p.sample_count >= 2)
      .sort((a: any, b: any) => a.avg_engagement_rate - b.avg_engagement_rate).slice(0, 3);

    const prompt = `You are a strategic LinkedIn content advisor. Based on the data below, recommend 3 specific posts the user should create next.

PERFORMANCE PATTERNS (what works):
${bestPatterns.map((p: any) => `${p.dimension}="${p.dimension_value}": ${p.avg_engagement_rate}% avg engagement (${p.sample_count} posts) [${p.confidence_level || "low"} confidence]`).join("\n") || "No patterns yet."}

UNDERPERFORMING PATTERNS:
${worstPatterns.map((p: any) => `${p.dimension}="${p.dimension_value}": ${p.avg_engagement_rate}% avg engagement (${p.sample_count} posts)`).join("\n") || "None."}

RECENT CONTENT (last 50 posts):
- Hook types used: ${JSON.stringify(hookCounts)}
- Tones: ${[...new Set(recentTags.map((t: any) => t.tone).filter(Boolean))].join(", ") || "none"}
...
PERSONAS:
${personas.map((p: any) => `- ${p.name} (${(p as any).industry || "General"}, ${(p as any).geography || "Global"}): pain points ${JSON.stringify(p.pain_points || [])}`).join("\n") || "No personas"}

${profile ? `BUSINESS: ${profile.company_summary || "N/A"} | Product: ${profile.product_summary || "N/A"} | Differentiators: ${JSON.stringify(profile.differentiators || [])}` : ""}

For each recommendation, classify as:
- "exploit" = double down on what's proven to work
- "fix" = address a weakness or underperformance
- "experiment" = test a new angle or combination
  ...
  "recommendations": [
    {
      ...
      "recommendation_type": "exploit|fix|experiment",
      "supporting_pattern": "the specific pattern data that supports this recommendation"
    }
  ],
  "gap_analysis": {
    "underused_hooks": ["..."],
    ...
    const aiResp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are a strategic LinkedIn content advisor. Give specific, data-driven recommendations. Classify each as exploit/fix/experiment." },
    ...
    for (const rec of (result.recommendations || [])) {
      await supabase.from("strategy_recommendations").insert({
        user_id: user.id, recommendation: rec, gap_analysis: result.gap_analysis || {},
