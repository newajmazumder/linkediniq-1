...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch post, context, and metrics
    const [postRes, ctxRes, metricsRes] = await Promise.all([
...
Return ONLY valid JSON with this EXACT structure:
{
  ...
  "writing_diagnosis": {
    "hook_analysis": {
      ...
      "summary": "<1-2 sentence hook assessment>"
    },
    "content_analysis": {
      "clarity": <number 1-10>,
    ...
    "what_weakened": ["<specific weakness 1>", "<specific weakness 2>"],
    "what_to_change": ["<specific actionable change 1>", "<specific actionable change 2>", "<specific actionable change 3>"]
  },
  "recommendations": {
    "what_to_repeat": ["<specific thing to repeat>", "<another>"],
    "what_to_avoid": ["<specific thing to avoid>", "<another>"],
    ...
    "strategy_suggestion": "<1-2 sentence strategy recommendation for similar future posts>"
  }
}
