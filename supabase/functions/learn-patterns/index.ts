...
function getConfidence(sampleCount: number): string {
...
serve(async (req) => {
  ...
  try {
      ...
      if (postIds.length > 0) {
        ...
        const newTags = (postsData || [])
          ...
          .map((p: any) => {
            ...
            return {
              user_id: user.id, post_id: p.id, draft_id: draft?.id || null,
              hook_type: p.hook_type || null, tone: p.tone || null, content_type: null,
              post_style: p.post_style || null, content_intent: p.content_intent || null,
              persona_id: p.persona_id || null, campaign_id: p.campaign_id || null,
              topic: null, cta_type: null, goal: null, word_count: wordCount, publish_hour: null,
            };
          });
    ...
    // ===== STEP 1b: Auto-tag linkedin_posts =====
    const { data: linkedinPostsWithMetrics } = await supabase
      .from("post_metrics").select("linkedin_post_id").eq("user_id", user.id);
    ...
    if (linkedinPostIds.length > 0) {
      const { data: existingLinkedinTags } = await supabase
        .from("content_tags").select("linkedin_post_id").eq("user_id", user.id).not("linkedin_post_id", "is", null);
      ...
      if (untaggedIds.length > 0 && LOVABLE_API_KEY) {
        const { data: untaggedPosts } = await supabase
          .from("linkedin_posts").select("id, content, publish_date").in("id", untaggedIds).limit(10);
        ...
        for (const ctx of (existingContexts || [])) contextMap[ctx.linkedin_post_id] = ctx;
        ...
        for (const lp of (untaggedPosts || [])) {
          ...
          if (ctx && ctx.hook_type) {
            await supabase.from("content_tags").insert({
              user_id: user.id, linkedin_post_id: lp.id,
              hook_type: ctx.hook_type, tone: ctx.tone, cta_type: ctx.cta_type,
              goal: ctx.goal, persona_id: ctx.persona_id, campaign_id: ctx.campaign_id,
              post_style: ctx.strategy_type, word_count: wordCount, publish_hour: publishHour,
            });
          } else {
            try {
              const classifyResp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
                ...
                body: JSON.stringify({
                  ...
                  messages: [
                    { role: "system", content: `Classify this LinkedIn post. Return VALID JSON only:
{"hook_type":"curiosity|contrarian|pain_driven|data_bold|question|story_opening","tone":"friendly|professional|empathetic|authoritative|bold|conversational","post_style":"founder_story|customer_story|educational|framework|pain_solution|product_insight|hybrid_story_insight|soft_promotion","cta_type":"soft|hard|question|dm_prompt|comment_prompt|link|none","content_intent":"Awareness|Education|Trust|Product|Lead","topic":"brief 2-3 word topic"}` },
              ...
              if (classifyResp.ok) {
                ...
                try {
                  ...
                  await supabase.from("content_tags").insert({
                    user_id: user.id, linkedin_post_id: lp.id,
                    hook_type: tags.hook_type, tone: tags.tone, post_style: tags.post_style,
                    cta_type: tags.cta_type, content_intent: tags.content_intent, topic: tags.topic,
                    goal: ctx?.goal, persona_id: ctx?.persona_id, campaign_id: ctx?.campaign_id,
                    word_count: wordCount, publish_hour: publishHour,
                  });
                } catch { /* skip */ }
              }
            } catch { /* skip */ }
    ...
    const { data: draftTags } = await supabase
      ...
      .eq("user_id", user.id).not("draft_id", "is", null);

    const { data: linkedinTags } = await supabase
      .from("content_tags")
      .select("*, post_metrics:linkedin_post_id(impressions, reactions, comments, reposts)")
      .eq("user_id", user.id).not("linkedin_post_id", "is", null);

    const enriched: any[] = [];

    for (const t of (draftTags || [])) {
      if (t.post_performance && t.post_performance.impressions > 0) {
        enriched.push({
          ...t,
    ...
    for (const t of (linkedinTags || [])) {
      if (t.post_metrics && t.post_metrics.impressions > 0) {
        enriched.push({
          ...t,
    ...
    const aggregateDimension = (dim: string, items: any[]) => {
    ...
    for (const dim of dimensions) {
      const groups = aggregateDimension(dim, enriched);
      const dimPatterns: any[] = [];
      ...
      for (const [value, items] of Object.entries(groups)) {
        ...
        dimPatterns.push({
          user_id: user.id, dimension: dim, dimension_value: value,
          sample_count: count, avg_impressions: Math.round(avgImpressions),
          avg_engagement_rate: Math.round(avgEngagement * 100) / 100,
          avg_likes: Math.round(avgLikes * 10) / 10, avg_comments: Math.round(avgComments * 10) / 10,
          best_combination: {}, insight: null,
          confidence_level: getConfidence(count), comparative_insight: null,
          updated_at: new Date().toISOString(),
        });
      }

      // Generate comparative insights within dimension
      if (dimPatterns.length > 1) {
        const sorted = [...dimPatterns].sort((a, b) => b.avg_engagement_rate - a.avg_engagement_rate);
        const best = sorted[0];
        const worst = sorted[sorted.length - 1];
        if (worst.avg_engagement_rate > 0) {
          const ratio = (best.avg_engagement_rate / worst.avg_engagement_rate).toFixed(1);
          best.comparative_insight = `Outperforms "${worst.dimension_value}" by ${ratio}x in engagement`;
          worst.comparative_insight = `Underperforms "${best.dimension_value}" by ${ratio}x — consider avoiding`;
        }
      }

      patterns.push(...dimPatterns);
    }

    // Distribution: post_length
    ...
    if (withWordCount.length > 0) {
      ...
      for (const [bucket, items] of Object.entries(lengthBuckets)) {
        ...
        const count = items.length;
        patterns.push({
          user_id: user.id, dimension: "post_length", dimension_value: bucket,
          sample_count: count,
    ...
    if (withPublishHour.length > 0) {
      ...
      for (const [bucket, items] of Object.entries(timeBuckets)) {
        ...
        const count = items.length;
        patterns.push({
          user_id: user.id, dimension: "publish_time", dimension_value: bucket,
          sample_count: count,
          avg_impressions: Math.round(items.reduce((s: number, i: any) => s + i.impressions, 0) / count),
          avg_engagement_rate: Math.round(items.reduce((s: number, i: any) => s + i.engagement_rate, 0) / count * 100) / 100,
          avg_likes: Math.round(items.reduce((s: number, i: any) => s + i.likes, 0) / count * 10) / 10,
    ...
    // ===== STEP 2b: Cross-dimensional aggregation (CTA×goal, CTA×persona) =====
    // CTA × Goal cross-dimension
    const ctaGoalGroups: Record<string, any[]> = {};
    for (const item of enriched) {
      if (!item.cta_type || !item.goal) continue;
      const key = `${item.cta_type}×${item.goal}`;
      if (!ctaGoalGroups[key]) ctaGoalGroups[key] = [];
      ctaGoalGroups[key].push(item);
    }
    for (const [key, items] of Object.entries(ctaGoalGroups)) {
      if (items.length < 2) continue;
      const count = items.length;
      patterns.push({
        user_id: user.id, dimension: "cta_x_goal", dimension_value: key,
        sample_count: count,
        avg_impressions: Math.round(items.reduce((s: number, i: any) => s + i.impressions, 0) / count),
        avg_engagement_rate: Math.round(items.reduce((s: number, i: any) => s + i.engagement_rate, 0) / count * 100) / 100,
        avg_likes: Math.round(items.reduce((s: number, i: any) => s + i.likes, 0) / count * 10) / 10,
    ...
    if (LOVABLE_API_KEY && patterns.length > 0) {
      try {
        const patternSummary = patterns.map(p =>
          `${p.dimension}="${p.dimension_value}": ${p.sample_count} posts, ${p.avg_engagement_rate}% engagement, ${p.avg_impressions} impressions [confidence: ${p.confidence_level}]`
        ).join("\n");
        ...
        const aiResp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
          ...
          body: JSON.stringify({
            model: "google/gemini-2.5-flash-lite",
            messages: [
              { role: "system", content: `Generate performance insights for LinkedIn content patterns. For each pattern, write a brief actionable insight. Also generate comparative insights where one pattern clearly outperforms another. Return VALID JSON only: {"insights": {"dimension|value": "insight text", ...}, "comparisons": ["pain_driven hooks outperform curiosity by 2.1x — prioritize pain framing", ...]}` },
        ...
        if (aiResp.ok) {
          ...
          try {
            ...
            for (const p of patterns) {
              ...
              if (parsed.insights?.[key]) p.insight = parsed.insights[key];
            }
          } catch { /* ignore */ }
        }
      } catch { /* AI optional */ }
