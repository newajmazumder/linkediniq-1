    ...
    <div className="content-fade-in space-y-6">
      ...
      {showAdd && (
        <div className="rounded-lg border border-border bg-card p-4 space-y-3">
          ...
          />
          <Input
            placeholder="Tags (comma-separated, e.g. chatbot, CX, AI)"
            value={newTags}
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">
              Paste LinkedIn posts below. Separate multiple posts with blank lines or ---.
            </p>
            <Textarea
            ...
            <div className="mt-3 flex items-center justify-between">
              <span className="text-xs text-muted-foreground">
                {pastedPosts.split(/\n{2,}|---/).filter((p) => p.trim().length > 20).length} post(s)
                detected
              </span>
              <Button
                size="sm"
                onClick={handleAnalyze}
                disabled={analyzing || !pastedPosts.trim()}
              >
                {analyzing ? (
                    ...
                    <div key={i} className="rounded-md bg-muted/50 p-3 space-y-1.5">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>
                          ...
                          <span className="text-foreground">{pa.hook_style}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Tone: </span>
                          <span className="text-foreground">{pa.tone}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Topic: </span>
                          <span className="text-foreground">{pa.topic}</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">CTA: </span>
                          <span className="text-foreground">{pa.cta_type}</span>
                        </div>
                      </div>
                    </div>
                  ))}
