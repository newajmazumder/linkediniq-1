...
// (Execution Score, Velocity Gap, Missed Posts, Conversion Signal, Predicted Outcome)
// Calm typographic hierarchy, no extra color noise.
...
const ExecutionDashboard = ({ campaignId, campaign, postPlans, weekCount, onChange }: Props) => {
    ...
    <div className="rounded-xl border border-border bg-card p-5 sm:p-6 space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-border border border-border rounded-md overflow-hidden">
        ...
        <Metric
          ...
          hint={m.velocityScore >= 1 ? "On pace" : `${Math.max(0, (m.velocityRequired - m.velocityActual)).toFixed(1)} short / wk`}
        ...
        <Metric
          ...
          value={`${predict.expected}`}
          suffix={campaign.target_quantity ? ` / ${campaign.target_quantity}` : ""}
          hint={`${predict.trajectory.replace("_", " ")} · ${predict.confidence} conf.`}
          ...
          <div className="space-y-2">
            {adaptations[0].adjustments?.map?.((a: any, i: number) => (
              <div key={i} className="rounded-md border border-border p-3">
                <p className="text-[10px] uppercase tracking-[0.1em] text-muted-foreground">{a.type?.replace(/_/g, " ")}</p>
                <p className="mt-0.5 text-sm font-medium text-foreground">{a.change}</p>
...
const Metric = ({ label, value, suffix, hint, hintTone = "muted" }: { label: string; value: string; suffix?: string; hint?: string; hintTone?: "muted" | "warn" }) => (
  <div className="px-4 py-3">
    ...
    {hint && (
      <p className={cn("mt-0.5 text-[11px] tabular-nums", hintTone === "warn" ? "text-yellow-600" : "text-muted-foreground")}>{hint}</p>
    )}
  </div>
);
